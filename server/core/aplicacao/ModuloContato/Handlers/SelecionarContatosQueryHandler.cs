...
public class SelecionarContatosQueryHandler(IRepositorioContato repositorioContato,
                                            ILogger<SelecionarContatosQueryHandler> logger)
    : IRequestHandler<SelecionarContatosQuery, Result<SelecionarContatosResult>>
{
    public async Task<Result<SelecionarContatosResult>> Handle(SelecionarContatosQuery query,
        ...
        try {
            ...
            var result = new SelecionarContatosResult(
                registros.Select(r => new SelecionarContatosDTO(
                    r.Id, r.Nome, r.Telefone, r.Email, r.Empresa, r.Cargo
                    )).ToImmutableList());
