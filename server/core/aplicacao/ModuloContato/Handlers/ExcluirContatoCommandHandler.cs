...
public class ExcluirContatoCommandHandler(IRepositorioContato repositorioContato,
                                          IUnitOfWork unitOfWork,
                                          ILogger<ExcluirContatoCommandHandler> logger)
    : IRequestHandler<ExcluirContatoCommand, Result<ExcluirContatoResult>>
{
    public async Task<Result<ExcluirContatoResult>> Handle(ExcluirContatoCommand command, 
		...
		try {
            ...
            logger.LogError(ex, "Ocorreu um erro durante a exclusão de {@Registro}.", command);
